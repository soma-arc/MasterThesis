...
the algorithm to visualize Kleinian groups called
...
We write transformations in lowercase alphabet and 
their inverse in uppercase alphabet.
For instance, we write $a$ as $f(z)$, $b$ as $g(z)$, $A$ as $f(z)^{-1}$,
and $B$ as $g(z)^{-1}$.

We write compositions of transformations as line upped letters.
For instance, $f(z) \circ g(z) \circ f(z)^{-1}$ is represented by $abA$.
...
in a sphere with infinite radius is the same as a reflection through a
plane.

Later, we consider Jacobian of the inversions.
Let $C$ be the center of the inversion circle, let $R$ be its radius,
and let $P$ be a point before applying the inversion in the circle.
$Jacobian = R^2 / distance(P,~C)^2$.
Note that the Jacobian of the inversions in a circle with
infinite radius is $1$.
In the same way as Jacobian of a circle inversion, we can derive Jacobian of
sphere inversions.
...
The nodes of the graph show words (compositions of elements of the group.)
...
breadth first search. We can obtain words from the short side.
...
We decide the maximum depth of traversing in advance, and we obtain
long words.
Next, we apply obtained word to the fixed points.
...
\noindent Pseudo-code of IIS is shown in Algorithm \ref{arg:iis2d}.
...
In regular ray tracing, we calculate the intersection algebraically, but
...
\begin{figure}[htbp]
 \begin{minipage}[t]{0.5\hsize}
  ...
  \caption{\textit{XY-slice image of Figure \ref{fig:simpleOrb}}}
  \label{fig:slice2d}
  \hspace*{\fill}
 \end{minipage}
 \begin{minipage}[t]{0.5\hsize}
...
The equation to calculate the Jacobian of an inversion is as follows.

Let $S$ be the center of the inversion sphere, let $R$ be its radius,
and let $P$ be a point before applying the inversion in $S$.
$Jacobian = R^2 / distance(P,~S)^2$.
Note that the Jacobian of the inversions in a sphere with infinite radius
is $1$.
...
IIS uses circle inversions on the other hand Jos Leys uses normal
...
Pixel Lookup} \cite{journals/combinatorics/GagernR09}.
The algorithm is for visualizing tiling.
His method is similar to IIS.
We explore the method not only tiling but also other varieties of images.