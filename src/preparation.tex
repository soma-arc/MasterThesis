...
whose center is $C$ and radius $R$ is $f(x) = distance(P, C) - R$.
...
There are well known technique to copy objects.
We can use modulo operator to repeat object.
See Figure n. There are a sphere and a ray.
We draw repeated spheres.
Apply modulo to the ray and compute distance between ray and original
sphere.

Next, we consider scaling.
There is one sphere and we want to render orbit of $y = 2x$.
We assume, nearest sphere to tip of the ray is 
We use jacobian to

However, we only know tip of the ray and original sphere.
Thus, we transform the ray.
do not preserve distance between ray and sphere.
Thus we use jacobian
