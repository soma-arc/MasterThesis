...
% Excluding the identical mapping, we will classify M\"obius transformations
...
% One is the case $|\lambda|=1$, 
% $f(z)$ is called an elliptic transformation.
...
% Remark that if a M\"obius transformation $f(z)$ has a finite order
% then it is elliptic.
...
% repelling fixed point. This is because the orbid space of $f(z)$ of an arbitrary point $z_0$(, excluding the fixed point set,)
...
% Also, $f(z)$ loxodromic if and only if $\mathrm{tr}^2X < 4$.
When the standard form is
$\begin{pmatrix}\lambda & 0 \\ 0 & \lambda^{-1} \end{pmatrix}$
...
% a modifier \textit{simple} or \textit{compound}.
...
% For detailed information of classfication of three dimensional M\"obius, see chapter 4.3.2.
